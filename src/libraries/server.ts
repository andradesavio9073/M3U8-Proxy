...
function getHandler(options, proxy) {
    ...
    return function(req, res) {
        ...
        if (!/^\/https?:/.test(req.url) && !isValidHostName(location.hostname)) {
            ...
            if (uri.pathname === "/m3u8-proxy") {
            ...
            } else if (uri.pathname === "/image-proxy") {
                ...
                const url = uri.searchParams.get("url");
                return proxyImage(url ?? "", headers, res);
            } else {
...
export default function server() {
    ...
    createServer({
        ...
        httpsOptions: process.env.USE_HTTPS ? {key: readFileSync(
            process.env.HTTPS_KEY || "privkey.pem"
          ),
          cert: readFileSync(
            process.env.HTTPS_CERT || "fullchain.pem"
          ),} : false
    }).listen(port, Number(host), function() {
        console.log(colors.green("Server running on ") + colors.blue(`${host}:${port}`) );
    });
}
...
export async function proxyImage(url: string, headers:any, res:http.ServerResponse) {
